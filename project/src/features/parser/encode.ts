...
export function splitBloxdschem(json: EncodedSchemJson): SplitResult {
  const schems: EncodedSchemJson[] = [];
...
export function writeBloxdSchem(json: RawSchemInput): WrittenSchems {
  const avroJson: EncodedSchemJson = {
    name: json.name,
    x: json.pos[0],
    y: json.pos[1],
    z: json.pos[2],
