...
let colorLUT: Uint8Array | undefined;
let blockIdMap: Uint16Array | undefined;
let terrainCanvas: HTMLCanvasElement | null = null;
let layerNameToId: Map<string, number>;
let layerIdMap: Uint8Array | undefined;
let layerColorLUT: Uint8Array;

export function updateTerrainCanvasTransform(): void {
...
export function updateTerrainRegion(x: number, y: number, w: number, h: number): void {
  updateBlockIdMapRegion(x, y, w, h);
  updateFullRegion(x, y, w, h);
  renderFullTerrain();
}

function updateBlockIdMapRegion(x0: number, y0: number, w0: number, h0: number): void {
  if (!blockIdMap || !mapState.map || !mapState.topBlockMap || !mapState.blockMap) return;
...
function compileShader(glCtx: WebGL2RenderingContext, type: number, source: string): WebGLShader {
...
function linkProgram(glCtx: WebGL2RenderingContext, vsSrc: string, fsSrc: string): WebGLProgram {
...
let fullGl: WebGL2RenderingContext | null = null;
let fullCanvas: OffscreenCanvas | null = null;
let terrainProgram: WebGLProgram | null = null;
let fullQuadVAO: WebGLVertexArrayObject | null = null;

let heightTexture: WebGLTexture | null = null;
let blockIdTexture: WebGLTexture | null = null;
let colorLUTTexture: WebGLTexture | null = null;
let layerIdTexture: WebGLTexture | null = null;
let layerColorLUTTexture: WebGLTexture | null = null;

let fullInitialized = false;

function checkMaxTextureSize(glCtx: WebGL2RenderingContext, width: number, height: number): void {
  const maxSize = glCtx.getParameter(glCtx.MAX_TEXTURE_SIZE) as number;
  if (width > maxSize || height > maxSize) {
    throw new Error(
      `Map size (${width}x${height}) exceeds MAX_TEXTURE_SIZE (${maxSize}). Tiling required.`
    );
  }
}
...
export function initTerrainCanvas(existingCanvas: HTMLCanvasElement): void {
...
function initFullTerrainRenderer(width: number, height: number): void {
  const ctx = terrainCanvas!.getContext("webgl2", { antialias: false, alpha: false });
  if (!ctx) throw new Error("WebGL2 not available for full terrain renderer");
  fullGl = ctx;
...
function uploadColorLUTToFullRenderer(): void {
  if (!fullGl || !colorLUTTexture || !colorLUT) return;
...
function uploadFullHeightMap(): void {
  if (!fullGl || !heightTexture || !mapState.map) return;
...
function uploadFullBlockIdMap(): void {
  if (!fullGl || !blockIdTexture || !blockIdMap) return;
  const h = sizeState.heightLength;
  const w = sizeState.widthLength;
...
function uploadLayerColorLUTToFullRenderer(): void {
  if (!fullGl || !layerColorLUTTexture || !layerColorLUT) return;
...
function uploadFullLayerIdMap(): void {
  if (!fullGl || !layerIdTexture || !layerIdMap) return;
  const h = sizeState.heightLength;
  const w = sizeState.widthLength;
 ...
 */
function updateFullRegion(x0: number, y0: number, w0: number, h0: number): void {
  if (!fullGl || !heightTexture || !blockIdTexture || !mapState.map || !blockIdMap) return;
...
function updateLayerIdMapRegion(x0: number, y0: number, w0: number, h0: number): void {
  if (!layerIdMap || !mapState.layerMap || !layerNameToId) return;
...
function updateLayerIdTextureRegion(x0: number, y0: number, w0: number, h0: number): void {
...
export function updateTerrainBlockRegion(x: number, y: number, w: number, h: number): void {
  updateBlockIdMapRegion(x, y, w, h);
  updateFullRegion(x, y, w, h);
  renderFullTerrain();
}

export function updateTerrainLayerRegion(x: number, y: number, w: number, h: number): void {
  updateLayerIdMapRegion(x, y, w, h);
  updateLayerIdTextureRegion(x, y, w, h);
  renderFullTerrain();
}

export function rebuildLayerPalette(): void {
  layerNameToId = buildLayerNameToId(layerColors);
  layerColorLUT = buildLayerColorLUT(layerColors, layerNameToId);
  uploadLayerColorLUTToFullRenderer();
...
export function renderFullTerrain(): void {
