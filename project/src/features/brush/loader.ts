...
import type { LoadedBrushes, BrushLoadResult, BrushType } from "../../core/types.js";

async function loadBrush(filename: string): Promise<BrushLoadResult> {
...
export async function loadAllBrushes(
  brushImages: string[],
  container: HTMLElement,
  brushBar: HTMLElement
): Promise<void> {
  showLoading();
  try {
    ...
    const loadedBrushes: LoadedBrushes = {};
    for (const { filename: name, normalized } of results) {
      ...
      button.addEventListener("click", () => {
        ...
        brushState.brushType = name as BrushType;
      });
    }
