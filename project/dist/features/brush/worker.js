// src/features/brush/brush.worker.ts
//
// メインスレッドとは完全に分離。DOMやmapStateには一切触れない。
// mapState.map と同じ SharedArrayBuffer 上の Float32Array を直接参照するので、
// ここで書き込んだ値はメインスレッド側にも即座に反映される(同じメモリ)。
const state = {
    map: null,
    width: 0,
    height: 0,
...
self.onmessage = (e) => {
    const msg = e.data;
    console.log("[worker] received", msg.type, msg.type === "applyBrush" ? msg.params : "");
    switch (msg.type) {
        case "init": {
            state.width = msg.width;
            state.height = msg.height;
            state.map = new Float32Array(msg.buffer);
            console.log("[worker] init done", { width: state.width, height: state.height, mapLength: state.map.length, isShared: msg.buffer instanceof SharedArrayBuffer });
            break;
        }
        case "applyBrush": {
            ...
            console.log("[worker] computed", { changedCount: changed.length, sample: changed[0] });
