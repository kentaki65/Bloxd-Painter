    ...
    for (let dy = -r; dy <= r; dy++) {
        ...
        for (let dx = -dxMax; dx <= dxMax; dx++) {
            ...
            if (rangeFilter.below.enabled && newH > rangeFilter.below.input)
                continue;
            if (rangeFilter.slopeAbove.enabled && d < rangeFilter.slopeAbove.input)
                continue;
            if (rangeFilter.slopeBelow.enabled && d > rangeFilter.slopeBelow.input)
                continue;
...
function getHeightFrom(map, x, y) {
    if (!map)
        return undefined;
    if (x < 0 || y < 0 || x >= state.width || y >= state.height)
        return undefined;
...
export function getSlopeAngleAt(z, x, source) {
