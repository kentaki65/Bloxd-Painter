...
function tryGetTopBlock(y, x) {
    return state.topBlockMap?.[y]?.[x];
}
function getBlock(yy, z, x) {
...
function getColorRGB(blockName) {
    ...
    const rgb = [base[0] ?? 0, base[1] ?? 0, base[2] ?? 0];
...
async function renderChunkToImageBitmap(cx, cy, cellSize, zoom) {
    ...
    if (cellsW <= 0 || cellsH <= 0)
        return null;
    if (!state.map || !state.layerMap || !state.topBlockMap || !state.blockMap)
        return null;
...
self.onmessage = async (e) => {
    ...
    switch (msg.type) {
        ...
        case "syncState": {
            if (msg.topBlockMap !== undefined)
                state.topBlockMap = msg.topBlockMap;
            if (msg.layerMap !== undefined)
                state.layerMap = msg.layerMap;
            if (msg.blockMap !== undefined)
                state.blockMap = msg.blockMap;
            if (msg.waterLevel !== undefined)
                state.waterLevel = msg.waterLevel;
        ...
        case "renderChunk": {
            ...
            self.postMessage({ type: "chunkResult", requestId: msg.requestId, cx: msg.cx, cy: msg.cy, bitmap }, bitmap ? [bitmap] : []);
            break;
        }
    }
};
