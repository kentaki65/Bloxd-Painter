...
function getColor(blockName) {
    if (colorCache.has(blockName))
        return colorCache.get(blockName);
...
export function draw(canvas) {
    ...
    for (let cy = startChunkY; cy < endChunkY; cy++) {
        for (let cx = startChunkX; cx < endChunkX; cx++) {
            ...
            ctx.drawImage(chunk, px, py, Math.round(chunk.width * cameraState.zoom), Math.round(chunk.height * cameraState.zoom));
        }
    }
